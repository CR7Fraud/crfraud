...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || !isHome ? 'bg-background/95 backdrop-blur-md border-b border-border shadow-sm' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-14 gap-4">

        {/* Logo */}
        <Link to="/" className="flex items-center gap-2.5 shrink-0">
          <div className="w-8 h-8 bg-zinc-900 rounded-lg flex items-center justify-center border border-zinc-700 shadow-md">
            <span className="font-playfair font-black text-sm leading-none">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button className="xl:hidden p-1.5" onClick={() => setMenuOpen(!menuOpen)}>
            <div className="w-5 h-4 flex flex-col justify-between">
              <span className={`block h-0.5 transition-all ${menuOpen ? 'rotate-45 translate-y-1.5' : ''} ${scrolled || !isHome ? 'bg-foreground' : 'bg-white'}`} />
              <span className={`block h-0.5 transition-all ${menuOpen ? 'opacity-0' : ''} ${scrolled || !isHome ? 'bg-foreground' : 'bg-white'}`} />
              <span className={`block h-0.5 transition-all ${menuOpen ? '-rotate-45 -translate-y-1.5' : ''} ${scrolled || !isHome ? 'bg-foreground' : 'bg-white'}`} />
            </div>
          </button>
        </div>
